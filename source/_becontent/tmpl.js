...
var checkDates = function(date) {
  ...
  if(beMultiData[selectorDeHoteles.selectedIndex]['fechas'].length > 0)
  {
    var today = new Date();
    var yyyy = today.getFullYear();

    var currentDateFilter = beMultiData[selectorDeHoteles.selectedIndex]['fechas'];
    or_blockDate_total = beMultiData[selectorDeHoteles.selectedIndex]['fechas'].length + 1;

    or_blockDate_in = [];
    or_blockDate_out = [];
    for (let o = 0; o < 2; o++) {
      yyyy = yyyy + o;
      for (let i = 0; i < currentDateFilter.length; i++) {
        const element = currentDateFilter[i];
        var inDate = yyyy + ' - ' + element[0];
        var outDate = element[1];
